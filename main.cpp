...
static const char* g_shader_fragment_quad = ""
                                            ...
                                            "else m.y/=u[3].y;"
                                            "vec3 "
                                            "h=normalize(u[1]),r=normalize(cross(h,u[2])),d="
                                            "normalize(m.x*r+m.y*normalize(cross(r,h))+h),c,e;"
...
void write_frame(unsigned screen_w, unsigned screen_h, unsigned idx)
{
    ...
    glReadPixels(0, 0, static_cast<GLsizei>(screen_w), static_cast<GLsizei>(screen_h), GL_RGB,
...
#endif
{
    ...
    bool egl_result = egl_init(
        reinterpret_cast<NativeWindowType>(&g_egl_native_window), &g_egl_display, &g_egl_surface);
#if defined(USE_LD)
    if (!egl_result) {
        teardown();
        exit(1);
    ...
    GLuint pipeline;
    GLuint program_vert
        = shrinky_glCreateShaderProgramv(GL_VERTEX_SHADER, 1, &g_shader_vertex_quad);
    g_program_fragment
        = shrinky_glCreateShaderProgramv(GL_FRAGMENT_SHADER, 1, &g_shader_fragment_quad);
...

    {
        ...
        // Example by "bst", taken from "Music from very short programs - the 3rd iteration" by
        // viznut.
    ...
    if (flag_record) {
        ...
        for (;;) {
            int ticks = static_cast<int>(
                static_cast<float>(frame_idx) / 60.0f * static_cast<float>(AUDIO_BYTERATE));
    ...
    for (;;) {
        ...
        if (g_flag_developer) {
            ...
            if (0 != mouse_look_x) {
                float angle
                    = static_cast<float>(mouse_look_x) / static_cast<float>(screen_h / 4) * 0.25f;
                float ca = cosf(angle);
                float sa = sinf(angle);
            ...
            if (0 != mouse_look_y) {
                float angle
                    = static_cast<float>(mouse_look_y) / static_cast<float>(screen_h / 4) * 0.25f;
                float ca = cosf(angle);
                float sa = sinf(angle);
        ...
        if (g_flag_developer) {
            current_time
                += static_cast<float>(AUDIO_BYTERATE) / 60.0f * static_cast<float>(time_delta);
...
int SHRINKY_MAIN(int argc, char** argv)
{
    ...
    try {
        if (argc > 0) {
            ...
            desc.add_options()("developer,d", "Developer mode.")("help,h", "Print help text.")(
                "record,R",
                "Do not play intro normally, instead save audio as .wav and frames as .png "
                "-files.")("resolution,r", po::value<std::string>(),
                "Resolution to use, specify as 'WIDTHxHEIGHT' or 'HEIGHTp'.")(
            ...
            if (vmap.count("resolution")) {
                boost::tie(screen_w, screen_h)
                    = parse_resolution(vmap["resolution"].as<std::string>());
            }
